...
which two necessary files "libgsl.dll" and "libslcblas.dll" are included in this
package. Make sure you put the two dll files in the same folder with BEAM,
or put them in your system folder (e.g., "c:\\windows\\system32"), otherwise the
program will not run.

For linux version, the GSL are supposedly included in the executables, so you can simply
put all files into one folder and just run the program by ./BEAM. However, we do
receive reports that linux version has GSL related problems. Please let us know
if you encounter problems when running the linux version, so that we can nail down 
the exact problem. One possibility is to install the newest version of GSL from
...
Please see "data.txt" for an example of the input file.
